...
public class FriendsGUI implements GUI {
    ...
    public FriendsGUI(Main plugin, User user, int page) {
        this.plugin = plugin;
        this.user = user;
        this.page = page;
    }
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (guiUse != null) {
            switch ((String) guiUse) {
                case "back":
                    player.openInventory(new FriendsGUI(plugin, this.user, page - 1).getInventory());
                    break;
                case "next":
                    player.openInventory(new FriendsGUI(plugin, this.user, page + 1).getInventory());
                    break;
                case "player":
                    User friend = plugin.getDatabaseManager().getUser(UUID.fromString((String) NbtUtils.getNBT(item, "player")));
                ...
                case "open_online_players_gui":
                    player.openInventory(new OnlinePlayersGUI(plugin, this.user, 0).getInventory());
                    break;
            }
        }
    ...
    public Inventory getInventory() {
        ...
        inventory.setItem(plugin.friendsGUIConfig.openOnlinePlayerGUI.slot, NbtUtils.setNBT(NbtUtils.setNBT(ItemBuilder.makeItem(plugin.friendsGUIConfig.openOnlinePlayerGUI), "gui_use", "open_online_players_gui"), "gui_protect", true));
